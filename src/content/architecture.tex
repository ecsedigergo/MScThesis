...
\begin{enumerate}[label=REQ-SOQ-\arabic*, leftmargin=*, format=\small]
	\item The Section Occupancy Query must collect the occupancy information together from all segments on the track.
\end{enumerate}

\begin{enumerate}[label=REQ-OCQ-\arabic*, leftmargin=*, format=\small]
	\item The Occupancy Query software element must determine the occupancy state for each segment.
	\item The Occupancy Query software element must send a message when an occupancy state changed for each segment. The component must send all the present section's occupancy states.
\end{enumerate}

\paragraph{Track segment controller elements}
\begin{enumerate}[label=REQ-GPIO-\arabic*, leftmargin=*, format=\small]
	\item The GPIO software element must change the GPIO pins between their input and output states.
\end{enumerate}

\begin{enumerate}[label=REQ-TEC-\arabic*, leftmargin=*, format=\small]
	\item The Track Element Controller must be able to enable and disable each section with a specific command.
	\item The Track Element Controller must be able to change each turnout's direction to straight or divergent state.
\end{enumerate}

\begin{enumerate}[label=REQ-TD-\arabic*, leftmargin=*, format=\small]
	\item The Train Detector must identify the length and speed of a specific train.
\end{enumerate}
...
\begin{enumerate}[label=REQ-DB-\arabic*, leftmargin=*, format=\small]
	...
	\item The Dashboard must be able to change each turnout state separately.
	\item The Dashboard must be able to enable and disable all segments on the track.
	\item The Dashboard must be able to enable and disable each segment on the track, separately.
\end{enumerate}

\todo[inline]{DB req can be compressed together?}

\begin{enumerate}[label=REQ-SL-\arabic*, leftmargin=*, format=\small]
	\item The Safety Logic software must avoid safety-critical scenarios on the track. \todo[inline]{Explain safety-critical scenarious here?}
...
\subsection{Component level test plans}

\paragraph{Purpose of component level testing}

\paragraph{Test item and their functionality}

\paragraph{Software risks issues}
...
\paragraph{Approach / Strategy}

\paragraph{Item pass / fail criteria}
